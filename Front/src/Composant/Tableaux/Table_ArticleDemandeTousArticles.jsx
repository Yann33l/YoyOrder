    ...
    try {
      ...
      for (const key in updatedData[rowIndex]) {
        if (updatedData[rowIndex][key] !== data[rowIndex][key]) {
          ...
          console.log(
            `changedDataOnly: clé ${key}, valeur initale :${data[rowIndex][key]}  valeur modif ${updatedData[rowIndex][key]}`
          );

          // Appliquer le formateur uniquement si la colonne est "date Demande" pour la mettre au format YYYY-MM-DD compréhensible par la base de données
          if (key === "date Demande") {
